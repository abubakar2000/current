...
func viewidea(w http.ResponseWriter, r *http.Request) {
	...
	if nb.Owners[len(nb.Owners)-1] != user {

		db, err := sql.Open("mysql", "root:wasif@tcp(127.0.0.1:3306)/ideamarket")
		if err != nil {
			panic(err.Error())
		}
		results, err := db.Query("select balance from user where username = '" + user + "'")
		if err != nil {
		...
		if user_balance > nb.Viewing_price {
			...
			results, err = db.Exec("update user set balance=balance-" + strconv.FormatFloat(nb.Viewing_price, 'E', -1, 32) + " where username = '" + user + "';")
			if err != nil {
				json.NewEncoder(w).Encode("Error")
				//panic(err.Error()) // proper error handling instead of panic in your app
				return
			...
			nb.Can_be_viewed_by = append(nb.Can_be_viewed_by, user)
...
func Updateuser(w http.ResponseWriter, r *http.Request) {
	...
	results, err := db.Exec("update user set password='" + res["password"].(string) + "',email='" + res["email"].(string) + "',phone_number='" + res["phone_number"].(string) + "'where username='" + res["username"].(string) + "';")
	if err != nil {
		json.NewEncoder(w).Encode("Error")
		//panic(err.Error()) // proper error handling instead of panic in your app
		return
...
func stop_bidding(w http.ResponseWriter, r *http.Request) {
	...
	if nb.Bidding == true {
		if nb.Highest_bidding_price > 0.0 {
			...
			json.NewEncoder(w).Encode("Bidding Stopped successfully!")
			fmt.Println("Bidding Stopped successfully!")
		}
	} else {
...
func bid(w http.ResponseWriter, r *http.Request) {
	...
	// Title of idea on which user wants to bid
	title := res["Title"].(string)
	// user who wants to bid
	bidder := res["username"].(string)
	// bidding price of user
	bidding_price := res["bidding_price"].(float64)
	...
	if BlockData_of_interest.Bidding == true {
		...
		results, err := db.Query("select balance from user where username = '" + bidder + "'")
		if err != nil {
			json.NewEncoder(w).Encode("ERR")
			json.NewEncoder(w).Encode("Invalid username!")
			fmt.Print("Invalid Username!")
		...
		if user_balance > bidding_price {

			if bidding_price >= BlockData_of_interest.Ownership_price {
				if bidding_price >= BlockData_of_interest.Highest_bidding_price {

					// Deduct amount from the current bidder.
					fmt.Println(strconv.FormatFloat(bidding_price, 'E', -1, 32))
					results2, err2 := db.Exec("update user set balance = balance-" + strconv.FormatFloat(bidding_price, 'E', -1, 32) + " where username = '" + bidder + "';")
					if err2 != nil {
						json.NewEncoder(w).Encode("Error")
					...
					results2, err2 = db.Exec("update user set balance = " + strconv.FormatFloat(bidding_price, 'E', -1, 32) + " where username = 'escrow';")
					if err2 != nil {
						json.NewEncoder(w).Encode("Error")
						fmt.Println("Error occured!")
						return
					...
					BlockData_of_interest.Highest_bidder = bidder
					BlockData_of_interest.Highest_bidding_price = bidding_price
...
func Signup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("\n\n\n\n\n")
	results, err := db.Exec("insert into user(username,password,balance,email,phone_number) values ('" + res["username"].(string) + "','" + res["password"].(string) + "',0,'" + res["email"].(string) + "','" + res["phone_number"].(string) + "');")
	if err != nil {
		json.NewEncoder(w).Encode("ERR")
		//panic(err.Error()) // proper error handling instead of panic in your app
		fmt.Print("Invalid Username!")
...
func Signin(w http.ResponseWriter, r *http.Request) {
	...
	results, err := db.Query("select * from user where username = '" + res["username"].(string) + "'")
	if err != nil {
		json.NewEncoder(w).Encode("ERR")
		fmt.Print("Invalid Username!")
	}
	...
	for results.Next() {
		...
		if usr1.Password == res["password"].(string) {
...
func Deleteuser(w http.ResponseWriter, r *http.Request) {
	...
	results, err := db.Exec("delete from user where username ='" + res["username"].(string) + "' and password='" + res["password"].(string) + "'")
	if err != nil {
		json.NewEncoder(w).Encode("Error")
		defer db.Close()
		return
